...
class Actions {
    constructor(config) {
        ...
        this.columnsWidthEqualize = (cm, dm) => {
            ...
            desktop.scrollCenterRange(Desktop.RangeImpl.fromRanges(visibleColumns[0], visibleColumns[visibleColumns.length - 1]));
        };
...
class Column {
    ...
    isVisible(visibleRange, fullyVisible) {
        if (fullyVisible) {
...
Column.minWidth = 40;
class Desktop {
    constructor(kwinDesktop, pinManager, config, getScreen, layoutConfig) {
...
class Grid {
    ...
    *getVisibleColumns(visibleRange, fullyVisible) {
    ...
    getVisibleColumnsWidth(visibleRange, fullyVisible) {
        let width = 0;
        let nVisible = 0;
...
class Window {
    constructor(client, column) {
        ...
        this.focusedState = {
            fullScreen: false,
            maximizedMode: 0 /* MaximizedMode.Unmaximized */,
        };
        this.skipArrange = false;
        this.column = column;
        column.onWindowAdded(this, true);
    }
    moveToColumn(targetColumn, bottom) {
...
class ClientManager {
    ...
    minimizeClient(kwinClient) {
        ...
        if (client.stateManager.getState() instanceof ClientState.Tiled) {
            client.stateManager.setState(() => new ClientState.TiledMinimized(this.world, client), kwinClient === this.lastFocusedClient);
        }
    }
...
class DesktopManager {
    constructor(pinManager, config, layoutConfig, currentActivity, currentDesktop) {
        this.pinManager = pinManager;
        this.config = config;
    ...
    addDesktop(activity, kwinDesktop) {
        const desktopKey = DesktopManager.getDesktopKey(activity, kwinDesktop);
        const desktop = new Desktop(kwinDesktop, this.pinManager, this.config, () => this.selectedScreen, this.layoutConfig);
...
class World {
    constructor(config) {
        ...
        }, layoutConfig, Workspace.currentActivity, Workspace.currentDesktop);
